...
function SakrebulosTavmjdomarisMoadgileebi() {
  ...
  useEffect(() => {
    ...
    const membersList = Object.entries(members).filter(news => news[1].geo.title.includes(search)).map((news) => 
        
             {
              if(news[1].geo.m_title.includes('საკრებულოს თავმჯდომარის მოადგილეები')){
                ...
                <Link to={`/sakrebulos-tavmjdomaris-pirveli-moadgile/${news[0]}`}>
                  ...
                  <div>
                    <span className='time'>{news[1].geo.title}</span>
                    <span className='header'>{news[1].geo.position}</span>
