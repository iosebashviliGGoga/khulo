...
function HeaderApi() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      /* DROPDOWN language */
      const data = [{id: 1,link: 'geo', label: "GE"}, {id: 2, link: 'eng', label: "ENG"}, {id: 3 ,link: 'ru', label: 'RU'}];
      ...
      <div className="topNav">
          ...
          <div className="topNav-items">
              <div className='topNav-last'>
               <div className='hover'>
                 <input type="text" placeholder='ძიება' style={  {borderBottom: search ? "1px solid white" : "",
                                                                  color: search ?  "white" : ""}} onChange={(e) => setSearch(e.target.value)}/>
                ...
                <div className="hoverButtons">
                 ...
                 { Object.entries(menu).map(qveItem =>{
                    ...
                    if(item[1].cat_id == qveItem[1].parent_id && qveItem[1].level == 2){
                     // console.log(qveItem)
                      return <Link to={`/${qveItem[1].slug}`} clsassName='aqtebi'
...
<div className="wrapperr" style={wraper? {height:'667px'} : {height : '50px'}} >
  <nav>
    <input type="checkbox" id="menu" name="menu" class="m-menu__checkbox" ref={ref} />
    <label className="m-menu__toggle" for="menu" onClick={() => setWraper((prev) => !prev)}>
    ...
    <div className="m-menu">
      ...
      {Object.entries(menu).length?   Object.entries(menu).map((item,index)=>{
        ...
        return <ul>
                  <li>
                     <label class="a-label__chevron" for={`item-${index+1}`}>
                     {language == 1 && item[1].name_geo} 
                     {language == 2 && item[1].name_eng}
                     {language == 3 && item[1].name_ru}
                     </label>
                     <input type="checkbox" id={`item-${index+1}`} name={`item-${index+1}`} class="m-menu__checkbox"/>
                      <div className="m-menu">
                        <div className="m-menu__header">
                        <label className="m-menu__toggle" for={`item-${index+1}`}>
                        <svg width="35" height="35" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="2" stroke-linecap="butt" stroke-linejoin="arcs">
                      ...
                      <ul>
                          {Object.entries(menu).length ?  Object.entries(menu).map(qveItem=>{
                              ...
                              if(item[1].cat_id == qveItem[1].parent_id && qveItem[1].level == 2){
                                ...
                                return <li>
                                          <label htmlFor="">
                                            <Link to={`/${qveItem[1].slug}`} onClick={[handleBurgerButton,  () => setWraper((prev) => !prev)]}>
